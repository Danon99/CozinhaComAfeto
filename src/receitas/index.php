		...
		<form method="post" id="user_form" enctype="multipart/form-data">
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<select class="form-control" name="categoria_id" id="categoria_id" aria-label="Default select example">
					...
					<textarea id="modo_preparo" name="modo_preparo"></textarea>
					<br />
					<label>Foto</label>
				...
				<div class="modal-footer">
					<input type="hidden" name="receita_id" id="receita_id" />
...
<script type="text/javascript" language="javascript" >
    ...
    ClassicEditor
		...
		ClassicEditor
        .create( document.querySelector( '#modo_preparo' ) )
        .catch( error => {
            console.error( error );
        } );
...
$(document).ready(function(){
	...
	$('#add_button').click(function(){
		$('#user_form')[0].reset();
		$('.modal-title').text("Adicionar Receita");
		$('#action').val("Salvar");
		$('#operacao').val("Salvar");
	...
	$(document).on('submit', '#user_form', function(event){
		...
		if(titulo != '' && ingredientes != '' && modo_preparo != '' && categoria_id != '')
		{
			$.ajax({
		...
		{
			alert("Título, ingredientes, modo de preparo e categoria, Obrigatórios");
		}
	});
	
	$(document).on('click', '.update', function(){
		var receita_id = $(this).attr("id");
		$.ajax({
			url:"busca_unica.php",
			method:"POST",
			...
			success:function(data)
			{
				...
				$('#modo_preparo').val(data.modo_preparo);
